...
public class MouseSlice : MonoBehaviour
{
    ...
    bool SliceObject(ref Plane slicePlane, GameObject obj, List<Transform> positiveObjects, List<Transform> negativeObjects)
    {
        ...
        var newMeshCollider = newObject.GetComponent<MeshCollider>();
        if (newMeshCollider == null)
            newMeshCollider = newObject.AddComponent<MeshCollider>();
        ...
        ReplaceMesh(mesh, biggerMesh, meshCollider);
        ReplaceMesh(newObjMesh, smallerMesh, newMeshCollider);
    ...
    void SeparateMeshes(List<Transform> positives, List<Transform> negatives, Vector3 worldPlaneNormal)
    {
        ...
        for (i = 0; i < positives.Count; ++i)
            positives[i].transform.DOMove(positives[i].transform.position + separationVector, 0.1f);
